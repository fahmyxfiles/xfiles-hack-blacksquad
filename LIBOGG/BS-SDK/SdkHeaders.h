...
#define GObjects			0x01851E0C
#define GNames				0x01863E54

using namespace std;
char *ConvertString(const wchar_t *in){
	char result[500];
	wcstombs(result, in, 500);
	return result;
}
...
{ 
	...
	{ 
		...
		{ 
		if ( ! strcmp ( ConvertString(this->Names()->Data[ NameCache ( i ) ]->Name), FindName ) ) 
			{ 
				Index = NameCache ( i ); 
				return; 
			} 
		...
		{ 
			...
			{ 
				if ( ! strcmp ( ConvertString(this->Names()->Data[ i ]->Name), FindName ) ) 
				{ 
					NameCache.Add ( i ); 
					Index = i; 
				} 
	...
	char* GetName() 
	{ 
		return ConvertString(this->Names()->Data[ Index ]->Name); 
	}; 
