...
// FahmyXF1les @ http://www.xfileshack.net
// This base undetected within any anti cheat currently
...
#include <cassert>
void SetMyDllHandle(HMODULE hDll);
DWORD HookProcessEvent(DWORD uObject, DWORD hkProcessEvent);
void LinkProcessEvent(DWORD uObject);



bool ValidateHDSN(char* CheckURL);
...
namespace toolkit
{
	...
	public:
		...
		VMTHook( void* ptr, uint vfuncs = 0 ) { _init( ptr, vfuncs ); }
		~VMTHook();
	...
	protected:
		DWORD _init( void* ptr, uint vfuncs );
		void _swap_tables();
